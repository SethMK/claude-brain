---
type: person
status: draft
last_updated: 2026-04-26
sources: ["[[2025-06-05-lennys-newsletter-krieger-anthropic-cpo]]", "[[2026-04-26-krieger-ai-coworker-era-youtube]]"]
---

# Mike Krieger

## Role

CPO at Anthropic. Co-founded Instagram. Now driving Anthropic's product strategy — [[Claude Code]], [[Cowork]], [[Skills]], agent platform. **The clearest voice in the company on product framing for non-developers.**

## Why I track him

- **Most quotable Anthropic exec for PM evangelism**: hard stats, clear analogies, business framing
- Defines [[Agent-native product]] — the framing that separates Anthropic's strategy from chatbot apps
- Talks at industry events frequently — surfaces where Anthropic is heading before docs catch up

## Key claims / patterns

- **90-95% of Claude's code is AI-written** (per [[2025-06-05-lennys-newsletter-krieger-anthropic-cpo]])
- **MCP is "the fastest-growing standard in tech history"** (same)
- **Product bottleneck shifted** from engineering capacity → decision-making + merge queue management
- **PM teams embedded with AI researchers** drive 10x more impact than teams building on public APIs
- **Most companies reach 90%+ AI code gen "within a year"** (~mid-2026)
- **"Don't trap intelligence in a chat box"** — recurring framing across talks

## Related

- [[Agent-native product]] — his framing
- [[MCP]] · [[Claude Code]] · [[Cowork]] — his products
- Other Anthropic voices to add: [[Dario Amodei]] _(wanted)_

## Recent sources

- [[2025-06-05-lennys-newsletter-krieger-anthropic-cpo]] — Lenny interview, hard stats
- [[2026-04-26-krieger-ai-coworker-era-youtube]] — AI Coworker Era talk (transcript pending)
